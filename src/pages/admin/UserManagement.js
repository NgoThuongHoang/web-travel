    ...
    const handleFormSubmit = async (values) => {
        try {
            const url = selectedCustomer
                ? `${API_BASE_URL}/${selectedCustomer.id}`
                : API_BASE_URL;
            ...
            if (!response.ok)
                throw new Error("Lỗi khi lưu thông tin khách hàng");
                ...
                {selectedCustomer && (
                    <div>
                        ...
                        <p>
                            ...
                            selectedCustomer.tours.length > 0
                                ? selectedCustomer.tours
                                      .map(
                                          (tour) =>
                                              `${tour.name} - Mã tour: ${tour.tour_code}`
                                      )
                                      .join(", ")
                                : "Chưa tham gia tour"}
                        </p>
