...
const OrderManagement = () => {
        ...
        try {
            ...
            const confirmPromises = selectedOrders.map(async (orderId) => {
                ...
                if (order.status !== "pending") {
                    return Promise.reject(
                        new Error(
                            `Đơn hàng ${orderId} không ở trạng thái "Chờ xác nhận"!`
                        )
                    );
                }
            ...
            results.forEach((result, index) => {
                if (result.status === "fulfilled") {
                    ...
                    errorMessages.push(
                        `Đơn hàng ${orderId}: ${result.reason.message}`
                    );
                }
            });
    ...
    const calculateTotalAmount = (
        ...
        const adultPrice =
            tour.prices?.find((p) => p.age_group === "Adult")?.price || 0;
        const childPrice =
            tour.prices?.find((p) => p.age_group === "Child")?.price || 0;
        const singleRoomPrice =
            tour.prices?.find((p) => p.age_group === "Adult")
                ?.single_room_price || 0;
    ...
    const handleFormSubmit = async (values) => {
        try {
            ...
            const formattedCustomers = (values.customers || []).map(
                (customer) => ({
                    full_name: customer.full_name,
                    gender: customer.gender,
                    birth_date: customer.birth_date
                        ? customer.birth_date.format("YYYY-MM-DD")
                        : null,
    ...
    const columns = [
        ...
        },
        {
            title: "Tour",
            render: (_, record) =>
                tours.find((t) => t.id === record.tour_id)?.name || "N/A",
        },
        {
            title: "Mã tour",
            render: (_, record) =>
                tours.find((t) => t.id === record.tour_id)?.tour_code || "N/A",
        },
        {
            title: "Ngày đặt",
    ...
    useEffect(() => {
        if (selectedOrder && showEditModal) {
            ...
            const specialRequestsArray = specialRequests
                .split(",")
                .map((item) => item.trim());
            const selectedNotes = specialRequestsArray.filter((item) =>
                predefinedNotes.includes(item)
            );
            const additionalNotes =
                specialRequestsArray.find(
                    (item) => !predefinedNotes.includes(item)
                ) || "";
            ...
            form.setFieldsValue({
                ...
                birth_date: selectedOrder.birth_date
                    ? moment(selectedOrder.birth_date)
                    : null,
                pickup_point: selectedOrder.pickup_point,
                tourId: selectedOrder.tour_id,
                status: selectedOrder.status,
                startDate: selectedOrder.start_date
                    ? moment(selectedOrder.start_date)
                    : null,
                endDate: selectedOrder.end_date
                    ? moment(selectedOrder.end_date)
                    : null,
        ...
        <div className="order-management">
                ...
                {selectedOrder && (
                    ...
                    >
                        <div style={{ flex: 1, paddingRight: "20px" }}>
                            <p>
                                <strong>Khách hàng (người đặt tour):</strong>{" "}
                                {selectedOrder.full_name || "N/A"}
                            </p>
                            <p>
                                <strong>Giới tính:</strong>{" "}
                                {selectedOrder.gender || "N/A"}
                            </p>
                            <p>
                                <strong>Ngày sinh:</strong>{" "}
                                {formatDate(selectedOrder.birth_date)}
                            </p>
                            <p>
                            ...
                            <ul>
                                ...
                                selectedOrder.customers.length > 0 ? (
                                    selectedOrder.customers
                                        .filter(
                                            (c) => c.traveler_type !== "Lead"
                                        ) // Loại bỏ khách hàng chính (Lead) khỏi danh sách người đi cùng
                                        .map((customer, index) => (
                                            <li key={index}>
                                                {customer.traveler_type}{" "}
                                                {index + 1}:{" "}
                                                {customer.full_name} (
                                                {customer.gender},{" "}
                                                {formatDate(
                        ...
                        <div style={{ flex: 1, paddingLeft: "20px" }}>
                            ...
                            <p>
                                <strong>Trẻ dưới 5 tuổi:</strong>{" "}
                                {selectedOrder.children_under_5 || 0}
                            </p>
                            <p>
                                <strong>Trẻ 5-11 tuổi:</strong>{" "}
                                {selectedOrder.children_5_11 || 0}
                            </p>
                            <p>
                                <strong>Số phòng đơn:</strong>{" "}
                                {selectedOrder.single_rooms || 0}
                            </p>
                            <p>
                            ...
                            <p>
                                ...
                                {selectedOrder.special_requests || "N/A"}
                            </p>
                            <p>
                                <strong>Điểm đón:</strong>{" "}
                                {selectedOrder.pickup_point || "N/A"}
                            </p>
                        </div>
                    </div>
                )}
            ...
            >
                <Form form={form} layout="vertical" onFinish={handleFormSubmit}>
                    <Typography.Title
                        level={4}
                        style={{ marginBottom: 16, color: "#003087" }}
                    >
                    ...
                    <div style={{ marginTop: 24 }}>
                        <Text>
                            Bằng cách nhấn vào nút "LƯU" dưới đây, Khách hàng
                            đồng ý rằng các Điều kiện điều khoản này sẽ được áp
                            dụng. Vui lòng đọc kỹ Điều kiện điều khoản trước khi
                            thực hiện chọn sử dụng dịch vụ.
                        </Text>
                        <Form.Item
                            name="terms_accepted"
                            valuePropName="checked"
                        ...
                        <Col span={12}>
                            ...
                            >
                                <Select placeholder="Chọn phương thức thanh toán">
                                    <Option value="Thanh toán tại văn phòng Lửa Việt">
