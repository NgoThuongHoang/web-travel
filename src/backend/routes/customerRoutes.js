...
router.get('/', ensurePool, async (req, res) => {
  try {
    ...
    const countQuery = `
      ...
      ${whereClause}
    `;
    const countResult = await req.app.locals.pool.request()
    ...
    for (let customer of customers) {
      if (customer.tours && customer.tours.length > 0) {
        for (let tour of customer.tours) {
          ...
          `;
          const orderCustomerResult = await req.app.locals.pool.request()
          ...
          `;
          const participantsResult = await req.app.locals.pool.request()
...
router.put('/:id', ensurePool, async (req, res) => {
  try {
    ...
    const customerCheck = await req.app.locals.pool.request()
      .input('id', sql.Int, parseInt(id))
      .query('SELECT * FROM [web_travel].[dbo].[customers] WHERE id = @id');
...
full_name, phone = @phone, email = @email, address = @address
        WHERE id = @id
      `);
